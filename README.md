...
Для команд cd, cat, history будет учитываться только первый аргумент после тела команды (все остальное, что введет пользователь, проигнорируется без выведения ошибки)

Добавлена команда help <command> для кайфа юзера (а вдруг не пользовался линуксом или макосью ни разу в жизни). Тесты для команды help не писал (а они необходимы?)

Интерактивная оболочка выполнена на английском языке.
